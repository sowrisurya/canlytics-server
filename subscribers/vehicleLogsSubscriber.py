...
class VehicleLogsSubscriber:
	def __init__(self, vehicle_ids, dids_list, timeout = 100):
		self.__device_ids = {
		...
		self.__mqtt_client = MQTTClientV2(self.callback)
	...
	def callback(self, message: str):
		...
		with REDIS_CLIENT.lock("vehicle_logs_lock", sleep = 0.01):
			if not self.__crnt_msg:
				return
			device_id, log_data = StatusGetter.diagonostic_callback(crnt_msg=self.__crnt_msg, data=message, add_to_influx=False)
	...
	async def wait_for_msgs_recvd(self):
		st = time.time()
		while self.__crnt_msg is not None and time.time() - st < self.__timeout:
			await asyncio.sleep(0.1)
		

	def publish(self, input_data_hex: str, diag_name: str, frame_id: str, device_id: str):
	...
	async def publish_all(self):
		for did in self.__dids_list:
			for device_id in self.__device_ids.keys():
				...
				self.publish(input_data_hex = did.hex_data, diag_name = did.diag_name, frame_id = did.frame_id, device_id = device_id)
